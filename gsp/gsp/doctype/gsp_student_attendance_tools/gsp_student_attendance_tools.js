...
frappe.ui.form.on('GSP Student Attendance Tools', {
		...
		if (frm.doc.school) {
			frappe.call({
				...
				callback: function (r) {
					...
					frm.set_value("school_name", data.school_name);
					frm.set_value("gsp_name", data.gsp_name);
					frm.set_value("attendance_class", "");
	...
	update_attendance: function (frm) {
		if (!(frm.doc.month)) {
			frappe.msgprint("Please select the month")
			return
		}
			...
			frappe.call({
				...
				callback: function (response) {
					...
					if (data == 1) {
						...
						frm.set_value("students_attendance", "");
						frm.set_value("month", "");
						frm.set_value("attendance_class", "");
						frm.set_value("working_days", "");
					}
				}
			})
		}
