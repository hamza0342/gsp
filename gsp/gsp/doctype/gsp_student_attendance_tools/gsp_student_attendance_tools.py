...
class GSPStudentAttendanceTools(Document):
	pass
...
def get_months():
	...
	month=frappe.db.sql("SELECT `gsp_month` FROM `tabGSP Monthly Attendance` WHERE `parent`='GSP-%s' and `release_stipend`=1 order by `idx`"%(default_year))
	for x in month:
		months.append(x[0])
	return months
...
@frappe.whitelist()
def set_attendance(data= None,month = None):
	...
	for row in data:
		...
		if student.docstatus == 2:
			continue
		attendance_name = frappe.db.sql("Select  name from `tabGSP Monthly Attendance` where parent = '%s' and gsp_month = '%s'"%(student.name,month))

		# if student.gsp_student_attendance:
		# 	inserted = False
		# 	for curr in student.gsp_student_attendance:
		# 		if curr.gsp_month != month:
		# 			continue
		# 		else:
		# 			curr.attendance=int(row['attendance'])
		# 			inserted = True
		# 	if not inserted:
		# 		student=student.append("gsp_student_attendance",{})	
		# 		student.gsp_month=month
		# 		student.attendance=int(row['attendance'])
		# else:
		# 	student=student.append("gsp_student_attendance",{})	
		# 	student.gsp_month=month
		# 	student.attendance=int(row['attendance'])
